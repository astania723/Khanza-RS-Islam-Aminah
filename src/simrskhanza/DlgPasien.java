...
private void AlamatMouseMoved(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_AlamatMouseMoved
    ...
    private void formWindowActivated(java.awt.event.WindowEvent evt) {//GEN-FIRST:event_formWindowActivated
        if(tampilkantni.equals("Yes")){
            if(this.getHeight()<640){   
                ...
                if(this.getWidth()<900){
                    Scroll1.setHorizontalScrollBarPolicy(javax.swing.ScrollPaneConstants.HORIZONTAL_SCROLLBAR_ALWAYS);                                
                    FormInput.setPreferredSize(new Dimension(890,PanelBiasa.HEIGHT));
                }else{
                    Scroll1.setHorizontalScrollBarPolicy(javax.swing.ScrollPaneConstants.HORIZONTAL_SCROLLBAR_NEVER);                
                }
            }
    ...
    private void btnUbahPasswordActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnUbahPasswordActionPerformed
        if(NoRekamMedisDipilih.getText().isEmpty()||NamaPasienDipilih.getText().isEmpty()){
    ...
    private void ppBerkasDigitalBtnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ppBerkasDigitalBtnPrintActionPerformed
        if(tabMode.getRowCount()==0){
            ...
            berkaspasien.setJudul("::[ Berkas Digital Perawatan ]::","berkasrawat/pages");
            try {
                if(akses.gethapus_berkas_digital_perawatan()==true){
                    berkaspasien.loadURL("http://"+koneksiDB.HOSTHYBRIDWEB()+":"+koneksiDB.PORTWEB()+"/"+koneksiDB.HYBRIDWEB()+"/"+"berkaspasien/login2.php?act=login&usere="+koneksiDB.USERHYBRIDWEB()+"&passwordte="+koneksiDB.PASHYBRIDWEB()+"&no_rkm_medis="+tbPasien.getValueAt(tbPasien.getSelectedRow(),1).toString());
                }else{
                    berkaspasien.loadURL("http://"+koneksiDB.HOSTHYBRIDWEB()+":"+koneksiDB.PORTWEB()+"/"+koneksiDB.HYBRIDWEB()+"/"+"berkaspasien/login2nonhapus.php?act=login&usere="+koneksiDB.USERHYBRIDWEB()+"&passwordte="+koneksiDB.PASHYBRIDWEB()+"&no_rkm_medis="+tbPasien.getValueAt(tbPasien.getSelectedRow(),1).toString());
                }
            } catch (Exception ex) {
            }

            berkaspasien.setSize(internalFrame1.getWidth()-20,internalFrame1.getHeight()-20);
    ...
    private void tampil() {    
        ...
        try{
            ...
            try{
                if(cmbHlm.getSelectedItem().toString().equals("Semua")){
                    if(Carialamat.getText().trim().isEmpty()&&TCari.getText().trim().isEmpty()){}else{
