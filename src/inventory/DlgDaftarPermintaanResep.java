...
public class DlgDaftarPermintaanResep extends javax.swing.JDialog {
    ...
    public DlgDaftarPermintaanResep(java.awt.Frame parent, boolean modal) {
        ...
        tabMode3=new DefaultTableModel(null,new Object[]{
                "No.Resep","Tgl.Peresepan","Jam Peresepan","No.Rawat","No.RM","Pasien","Dokter Peresep","DPJP Ranap Utama",
                ...
                if(cmbStatus.getSelectedItem().toString().equals("Semua")){
                    while(rs.next()){
                        if(!rs.getString("no_rawat").isEmpty()){
                            dpjputama=Sequel.cariIsi("select dokter.nm_dokter from dpjp_ranap inner join dokter on dpjp_ranap.kd_dokter=dokter.kd_dokter where dpjp_ranap.no_rawat=?", rs.getString("no_rawat"));
                            if(dpjputama.isEmpty()){
                                dpjputama=Sequel.cariIsi("select dokter.nm_dokter from reg_periksa inner join dokter on reg_periksa.kd_dokter=dokter.kd_dokter where reg_periksa.no_rawat=?", rs.getString("no_rawat"));
                            }else{
                                dpjputama=dpjputama;
                            }
                        }
                        tabMode3.addRow(new String[]{
                            rs.getString("no_resep"),rs.getString("tgl_peresepan"),rs.getString("jam_peresepan"),rs.getString("no_rawat"),
                            rs.getString("no_rkm_medis"),rs.getString("nm_pasien"),rs.getString("nm_dokter"),dpjputama,rs.getString("status"),
