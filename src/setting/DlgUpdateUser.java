...
public class DlgUpdateUser extends javax.swing.JDialog {
    ...
    private boolean penyakit=false,obat_penyakit=false,dokter=false,jadwal_praktek=false,petugas=false,pasien=false,registrasi=false,tindakan_ralan=false,kamar_inap=false,
            ...
            satu_sehat_kirim_observation_lab=false,satu_sehat_kirim_observation_labmb=false,pemantauan_pews_maternal=false,validasi_sbar=false;
    ...
    private void tampil(String user) {  
        ...
        try{    
            ...
            ps=koneksi.prepareStatement(
                ...
                "user.satu_sehat_kirim_observation_lab,user.satu_sehat_kirim_observation_labmb,user.pemantauan_pews_maternal,user.validasi_sbar from user where user.id_user=AES_ENCRYPT(?,'nur')");
            try {
                ps.setString(1,user);
                rs=ps.executeQuery();
                if(rs.next()){
