                    ...
                    if ($header['X-Token'] == $token) {
                        ...
                        if(!empty($errors)) {
                            ...
                            if (JumlahBaris($result) !== 0) {
                                $sql2 = "SELECT * FROM tagihan_bpd_papua WHERE no_rkm_medis = '$decode[no_rkm_medis]' AND status_bayar = 'Pending'";
        ...
        case "bayar":
                    ...
                    if ($header['X-Token'] == $token) {
                        ...
                        if(!empty($errors)) {
                            ...
                            $sql = "UPDATE tagihan_bpd_papua SET status_bayar='Sudah',diupdatebank=current_time(),referensi='$decode[referensi]' WHERE no_rkm_medis = '$decode[no_rkm_medis]' AND no_nota = '$decode[no_nota]'";
                            if(bukaquery($sql)){
                                $response = array(
                                    'metadata' => array(
